...
class ControlNetSD3Model(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        if guess_mode:
            scales = torch.logspace(-1, 0, len(controlnet_block_res_samples), device=controlnet_block_res_samples[0].device)  # 0.1 to 1.0
            scales = scales * conditioning_scale
            controlnet_block_res_samples = [sample * scale for sample, scale in zip(controlnet_block_res_samples, scales)]
        else:
            controlnet_block_res_samples = [sample * conditioning_scale for sample in controlnet_block_res_samples]
